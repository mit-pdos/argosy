...
logRead a = _LoggingTwoDiskRefinement__compile (coq_Call (TxnD__Coq_op_read a))

logWrite
  :: Coq_addr
  -> Coq_block
  -> Coq_proc (TwoDisk__Op GHC.Base.Any) TxnD__WriteStatus
logWrite a b =
  _LoggingTwoDiskRefinement__compile (coq_Call (TxnD__Coq_op_write a b))

logSize :: Coq_proc (TwoDisk__Op GHC.Base.Any) Integer
logSize = _LoggingTwoDiskRefinement__compile (coq_Call TxnD__Coq_op_size)

logCommit :: Coq_proc (TwoDisk__Op GHC.Base.Any) ()
logCommit = _LoggingTwoDiskRefinement__compile (coq_Call TxnD__Coq_op_commit)
