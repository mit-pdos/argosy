...
newEnv fn0 fn1 = pure Env <*> openFile fn0 <*> openFile fn1
 where
  openFile :: FilePath -> IO CachedHandle
  openFile path = catchIOError
    (do
      fd <- openFd path ReadWrite Nothing defaultFileFlags
      return (path, Just fd)
    )
    (\e -> if isDoesNotExistError e then return (path, Nothing) else ioError e)
