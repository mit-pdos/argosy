...
td_read d a = ifExists d $ \fd -> do
  sz <- getSizeBlocks fd
  if a < sz
    then fdPread fd blocksize (fromIntegral $ addrToOffset a)
    else return (BS.replicate blocksize 0)
...
init fn0 fn1 sizeBytes = do
  ...
  when (not exists0 && not exists1) $ forM_ [fn0, fn1] $ \p ->
    withFile p WriteMode $ \h -> hSetFileSize h sizeBytes
  newEnv fn0 fn1
