...
import           Transactions.LogEncoding       ( encodeNum
                                                , decodeNum
                                                )
...
parseDiskPaths = liftA2
  ...
  (option
    auto
    (long "disk0" <> help "filename for first disk" <> showDefault <> value
      "disk0.img"
    )
  )
  (option
    auto
    (long "disk1" <> help "filename for second disk" <> showDefault <> value
      "disk1.img"
    )
  )
...
options = do
  ...
  cmd    <- hsubparser
    (  command
        ...
        (info
          (Init <$> initOptions)
          (progDesc "initialize replicated disks" <> footer diskDefaultMessage)
        )
    <> command
         "write"
         (info (Write <$> writeOptions)
               (progDesc "add to current transaction")
         )
    <> command
         "read"
         (info (Read <$> readOptions) (progDesc "read from committed state"))
    <> command
         "commit"
         (info (Commit <$> commitOptions)
               (progDesc "commit current transaction")
         )
    <> command "recover"
               (info (Recover <$> recoverOptions) (progDesc "run recovery"))
    )
  pure (common, cmd)
...
main = execParser opts >>= run
 where
  opts = info
    (options <**> helper)
    (fullDesc <> progDesc "logging over replicated disks" <> header
      "logging-client: example of using logging over replication"
    )

run :: Options -> IO ()
run (CommonOpts { diskPaths = (fn0, fn1) }, Init InitOptions { defaultSizeKB = size })
  = do
...
run (opts, Read ReadOptions {..}) =
  let a = fromIntegral readAddr
  in  runCmd opts $ do
        v <- TD.interpret (logRead a)
        liftIO $ print (decodeNum v)
run (opts, Commit CommitOptions  ) = runCmd opts $ TD.interpret logCommit
run (opts, Recover RecoverOptions) = runCmd opts $ TD.interpret logRecover
